...
gen_erl_body() ->
    ...
    {Str, MapKeyStr} = gen_erl_body(#str{}, #{}, Tabs),
    ...
    Str0 ++ Str1 ++ Str2 ++ MapKeyStr ++ Str3 ++ Str4 ++ Str5.

gen_erl_body(Str, RecordMap, []) ->
    Str1 = maps:fold(
        fun(Record, _, Acc) ->
            case record(Record) of
                #table{key = Key} when Key =/= undefined ->
                    Acc ++ "get_map_key(" ++ Record ++ ") -> " ++ Key ++ ";\r\n";
                _ ->
                    Acc
            end
        end, "", RecordMap),
    NewStr1 = Str1 ++ "get_map_key(_) -> undefined.\r\n\r\n",
    {Str, NewStr1};
...
gen_hrl_field(#field{name = Name, type = ?MAP, sub_type = SubType}, Map) ->
    #table{key = Key} = record(SubType),
    Str = "\t" ++ Name ++ " = #{}",
    {maps:put(SubType, 1, Map), Str, "% #{#" ++ SubType ++ "." ++ Key ++ " => #" ++ SubType ++ "{}}\r\n"};
