...
role_tabs() ->
	[db_role, db_role_friend, db_role_pack].
...
get_map_key(r_item) -> item_id;
get_map_key(r_package) -> type;
get_map_key(_) -> undefined.
...
get_fields(Record) when is_record(Record, db_role_pack) ->
	#db_role_pack{role_id = F1, package_map = F2} = Record,
	[
		#field{name = role_id, type = int, sub_type = undefined, value = F1},
		#field{name = package_map, type = map, sub_type = r_package, value = F2}
	];
...
get_fields(Record) when is_record(Record, r_package) ->
	#r_package{type = F1, item_map = F2} = Record,
	[
		#field{name = type, type = int, sub_type = undefined, value = F1},
		#field{name = item_map, type = map, sub_type = r_item, value = F2}
	];
get_fields(Record) when is_record(Record, r_item) ->
	#r_item{item_id = F1, amount = F2} = Record,
	[
		#field{name = item_id, type = int, sub_type = undefined, value = F1},
		#field{name = amount, type = int, sub_type = undefined, value = F2}
	];
get_fields(_) ->
	[].
...
get_field_map(db_role_pack) ->
	#{
		role_id => #field{name = role_id, type = int, sub_type = undefined},
		package_map => #field{name = package_map, type = map, sub_type = r_package}
	};
...
get_field_map(r_package) ->
	#{
		type => #field{name = type, type = int, sub_type = undefined},
		item_map => #field{name = item_map, type = map, sub_type = r_item}
	};
get_field_map(r_item) ->
	#{
		item_id => #field{name = item_id, type = int, sub_type = undefined},
		amount => #field{name = amount, type = int, sub_type = undefined}
	};
get_field_map(_) ->
	#{}.
...
field_map_to_record(db_role_pack, FieldMap) ->
	#db_role_pack{
		role_id = get_field_value(role_id, FieldMap),
		package_map = get_field_value(package_map, FieldMap)
	};
...
field_map_to_record(r_package, FieldMap) ->
	#r_package{
		type = get_field_value(type, FieldMap),
		item_map = get_field_value(item_map, FieldMap)
	};
field_map_to_record(r_item, FieldMap) ->
	#r_item{
		item_id = get_field_value(item_id, FieldMap),
		amount = get_field_value(amount, FieldMap)
	};
